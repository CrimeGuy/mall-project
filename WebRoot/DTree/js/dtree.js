...
dTree.prototype.checkedNodes = function() {
	...
	for (var i = 0; i < val.length; i++) {
		o = document.getElementById('c_' + this.obj + val[i]);
		if (o) {
			o.checked = true;
		}
	}
...
dTree.prototype.co = function(id) {
	...
	for (var n = 0; n < this.aNodes.length; n++) {
		if (this.aNodes[n].id == id) {
			this.cbCollection[id] = document
					.getElementById("c" + this.obj + id);
			break;
		}
	}
	return this.cbCollection[id];
...
// 获取选择的节点ID
dTree.prototype.getTextID = function() {
	var value = new Array();
	var cko;// checkobject
	for (var n = 0; n < this.aNodes.length; n++) {
		cko = this.co(this.aNodes[n].id);
...
dTree.prototype.node = function(node, nodeId) {
	...
	if (this.config.useIcons) {
		...
		if (this.config.check == true) {
			str += '<input type="checkbox" value=' + node.id + ' name="c'
...
dTree.prototype.getCookie = function(cookieName) {
	...
	if (posName != -1) {
		...
		if (endPos != -1)
			cookieValue = (document.cookie.substring(posValue, endPos));
